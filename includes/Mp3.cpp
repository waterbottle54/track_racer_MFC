...
long Mp3::GetState() const
{
	if (ready&&pimc)
	{
		OAFilterState fs;
		HRESULT hr = pimc->GetState(0, &fs);
		if (SUCCEEDED(hr))
			return ((long)fs);
	}
	return false;
...
bool Mp3::IsRunning() const
{
	return (GetState() == State_Running);
}

bool Mp3::IsPaused() const
{
	return (GetState() == State_Paused);
}

bool Mp3::IsStopped() const
{
...
bool Mp3::SetCurrentPosition(__int64 pos)
{
	return SetPositions(&pos, &duration, true);
}
...
bool Mp3::IsReady() const
{
	return ready;
}

bool Mp3::IsCompleted(int remain)
{
	if (GetCurrentPosition() != -1)
	{
		return (GetCurrentPosition() >= GetDuration() - remain);
	}
	return false;
}
